...
class MumimoInitService:
    ...
    def initialize_config(self) -> Config:
        ...
        cfg_instance: Config = config_utils.initialize_mumimo_config(self._get_sys_args().get(SYS_ARGS.SYS_CONFIG_FILE))
    ...
    def _get_prioritized_client_env_options(self) -> Dict[str, Any]:
        ...
        available_env_file: Optional[str] = self._get_sys_args().get(SYS_ARGS.SYS_ENV_FILE, None) or ".env"
        ...
        sys_tokens = self._get_sys_args().get(SYS_ARGS.SYS_TOKENS) or env_args.get(ENV_ARGS.ENV_TOKENS)
        ...
        prioritized_options.update(
            {
                SYS_ARGS.SYS_HOST: self._get_sys_args().get(SYS_ARGS.SYS_HOST) or env_args.get(ENV_ARGS.ENV_HOST),
                SYS_ARGS.SYS_PORT: self._get_sys_args().get(SYS_ARGS.SYS_PORT) or env_args.get(ENV_ARGS.ENV_PORT),
                SYS_ARGS.SYS_USER: self._get_sys_args().get(SYS_ARGS.SYS_USER) or env_args.get(ENV_ARGS.ENV_USER),
                SYS_ARGS.SYS_PASS: self._get_sys_args().get(SYS_ARGS.SYS_PASS) or env_args.get(ENV_ARGS.ENV_PASS),
                SYS_ARGS.SYS_CERT: self._get_sys_args().get(SYS_ARGS.SYS_CERT) or env_args.get(ENV_ARGS.ENV_CERT),
                SYS_ARGS.SYS_KEY: self._get_sys_args().get(SYS_ARGS.SYS_KEY) or env_args.get(ENV_ARGS.ENV_KEY),
                SYS_ARGS.SYS_SUPER_USER: self._get_sys_args().get(SYS_ARGS.SYS_SUPER_USER) or env_args.get(ENV_ARGS.ENV_SUPER_USER),
            }
        )
